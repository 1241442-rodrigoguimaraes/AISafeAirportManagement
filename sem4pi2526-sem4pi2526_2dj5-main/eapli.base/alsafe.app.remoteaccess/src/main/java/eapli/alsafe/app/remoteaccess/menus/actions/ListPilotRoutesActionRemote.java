...
public class ListPilotRoutesActionRemote implements Action {

    private final TCPClient client;

    public ListPilotRoutesActionRemote(final TCPClient client) {
        this.client = client;
    }
    ...
    public boolean execute() {
        try {
            client.sendPacket(new Packet((byte) 20, 0, new byte[0]));
            ...
            printEntries("Available Routes", payload);
            return true;
        } catch (final IOException e) {
            System.out.println("Error listing routes: " + e.getMessage());
            return false;
        }
    }
