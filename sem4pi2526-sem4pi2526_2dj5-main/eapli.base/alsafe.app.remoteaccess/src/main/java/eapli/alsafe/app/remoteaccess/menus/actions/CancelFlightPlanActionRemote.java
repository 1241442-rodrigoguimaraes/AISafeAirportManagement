...
public class CancelFlightPlanActionRemote implements Action {

    private final TCPClient client;

    public CancelFlightPlanActionRemote(final TCPClient client) {
        this.client = client;
    }
    ...
    public boolean execute() {
        try {
            ...
            final String flightPlanId = Console.readNonEmptyLine("Flight plan ID to cancel: ", "Flight plan ID is required").trim();
            final byte[] payload = flightPlanId.getBytes(StandardCharsets.UTF_8);
            client.sendPacket(new Packet((byte) 25, payload.length, payload));
            ...
            System.out.println("Error cancelling flight plan: " + e.getMessage());
            return false;
        }
    }
}
